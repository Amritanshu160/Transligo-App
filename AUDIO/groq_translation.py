...
def groq_translate(query, to_language):
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {
                "role": "system",
                "content": f"You are a helpful assistant that translates text from its original language to {to_language}."
            ...
            {
                "role": "user",
                "content": f"Translate '{query}' from its original language to {to_language}."
            }
        ],
